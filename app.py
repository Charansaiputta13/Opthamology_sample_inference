...
with tab_single:
    ...
    st.write("Upload a retinal fundus image for DR classification with Grad-CAM explainability.")
    ...
    if uploaded_file is not None:
        ...
        if img_pil is None:
            ...
            if not result['success']:
                ...
                if needs_review:
                    st.error(f"⚠️ **NEEDS REVIEW** - Low confidence or quality issues detected")
                else:
                ...
                with col_viz:
                    if show_gradcam and result.get('gradcam_heatmap') is not None:
                        st.subheader("🔥 Grad-CAM Explanation")
                        fig = model.create_gradcam_figure(
                            img_pil,
...
with tab_about:
    ...
    This is a production-ready AI system for automated Diabetic Retinopathy (DR) screening 
    ...
    >
    > Use of this tool implies acceptance of these limitations.
    """)
    ...
    **Single Image Mode:**
    ...
    3. Enable Grad-CAM for explainability
    4. View prediction and quality metrics
    ...
    **Evaluation Mode:**
    - View pre-computed evaluation metrics
    - Requires running evaluation script separately
    ...
    This system was developed for educational purposes as a demonstration of 
    production-quality ML engineering practices.
    
    **Keywords:** Diabetic Retinopathy, Medical AI, Deep Learning, Computer Vision, 
    Explainable AI, Grad-CAM, MobileNetV2
    """)
...
st.caption(f"🤖 Model: {Path(config.MODEL_PATH).name} | 📦 Version: 1.0.0")
