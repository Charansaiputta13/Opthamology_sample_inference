...
class Config:
    """
    Centralized configuration for DR classification system.
    ...
    MODEL_PATH: Path = Path(os.getenv(
        "MODEL_PATH",
        str(BASE_DIR / "pretrained" / "dr_mobilenetv2_5class.h5")
    ))
    ...
    # GRAD-CAM SETTINGS
    # =====================
    GRADCAM_LAYER_NAME: str = "Conv_1"  # Default last conv layer for MobileNetV2
    GRADCAM_ALPHA: float = 0.4  # Overlay transparency
    ...
    def _validate_paths(self) -> None:
        ...
        if not self.MODEL_PATH.exists():
            raise FileNotFoundError(
                ...
                f"Please ensure the trained model is present or set MODEL_PATH environment variable."
            )
        
        # DATA_DIR is optional (only needed for evaluation)
        # Don't raise error if missing
